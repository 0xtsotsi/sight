...
// room to drop down), spanning the panel's full width. Portaled to <body> so it overlays
// everything — but the panel body renders at a 0.75 `zoom` (see .app_root.is-compact
// .app_body) which a body-portaled element escapes, so the content re-applies that zoom
// (via .embed-editor_layer-popover-box) to match the panel's scale. The outer wrapper is
// unzoomed so its fixed-position coordinates stay in real screen pixels. Closes on
...
export default function LayerPopover({ anchorEl, onClose, ariaLabel, children }: {
  ...
  return createPortal(
    <div
      ...
      style={{ position: 'fixed', left: 0, right: 0, top: top ?? 0, visibility: top == null ? 'hidden' : 'visible' }}
    >
